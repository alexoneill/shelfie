...
import urllib
import string
# valid = ''.join([chr(i) for i in xrange(256)])
...
def recommendations(isbn):
	...
	for res in soup.find_all(class_="a-carousel-card a-float-left"):
		rfa = res.find_all('a')
		title_raw = rfa[0].find_all('div')[-1].contents[0].encode('ascii', 'ignore')
		title = ' '.join(''.join(title_raw.split('\n')).split()).encode('ascii', 'ignore')
		author = rfa[1].contents[0].encode('ascii', 'ignore')
		# rating = rfa[2].contents[1].span.contents[0]
		reclink = 'www.amazon.com%s'%rfa[0]['href'].encode('ascii', 'ignore')
